...
func (ds *DiscoveryServer) RegisterPeer(ctx context.Context, req *pb.RegisterPeerRequest) (*pb.RegisterPeerResponse, error) {
	...
	logger.Debug().Msg("Into ds.RegisterPeer")
	...
	ds.BLSkeyGenOnce.Do(ds.generateBLSKeys)
	...
	return &pb.RegisterPeerResponse{
		...
		BlsPubKey:        ds.BLSPublicKey,
		BlsPrivKeyShares: ds.BLSPrivateKeyShares[newID],
		BlsId:            ds.BLSIds[newID],
		BlsPubKeyShares:  ds.BLSPublicKeyShares,
	}, nil
...
// Generates keys for the TBLS threshold cryptosystem
...
// Generates keys for the TBLS threshold cryptosystem
func (ds *DiscoveryServer) generateBLSKeys() {
	n := len(ds.peerIdentities)
	f := (n - 1) / 3
	t := 2*f + 1
	...
	pubKeyShares := make([]*pb.PubKeyMsg, 0, 0)
	for _, j := range pubKeyShares_ {
		pubKeyShares = append(pubKeyShares, &pb.PubKeyMsg{BlsPubKey: j})
	}
	ds.BLSPublicKeyShares = pubKeyShares
